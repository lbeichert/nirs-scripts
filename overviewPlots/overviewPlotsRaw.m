% OVERVIEWPLOTSRAW create overview plots of unprocessed piglet data and save them to files
% 
...
% load unprocessed data if not already there
if ~exist('pigDataNTBRaw', 'var')
    datadir = getappdata(0, 'pigletdatadir');
    DataRaw = load([datadir,filesep,'argon-dex',filesep,'output',filesep,'cooling',filesep,'pigDataRaw']);
    pigDataNTBRaw = DataRaw.pigDataNTB;
end

% location the figures are saved to
outputDir = 'output/overviewPlotsRaw';
if ~exist(outputDir, 'dir')
    mkdir(outputDir);
end
 
for N = 1:length(pigDataNTBRaw)
    S = pigDataNTBRaw(N);
    ...
    f = figure('name', [pigN, ': Hb, CtOx (',group,')','_Raw']);  
    ...
    printFigure(f, [outputDir,filesep,pigN,'_',group,'_Raw'])
    saveas(f, [outputDir,filesep,pigN,'_', group,'_Raw'], 'fig');
end